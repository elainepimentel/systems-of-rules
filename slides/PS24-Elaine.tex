...
Gentzen: consistency of the arithmetic without complete induction.
\bigskip

\begin{tabular}{lcl}
\includegraphics[scale=0.5]{figs/gentzen1} &\qquad& \includegraphics[scale=0.3]{figs/gentzen3}\\
\includegraphics[scale=0.5]{figs/gentzen2}& & %{\small Credit: Eckhart Menzler-Trott}
\end{tabular}
}	
\only<3>{
\begin{center}
...
\[
\frak{U}_1,\ldots \frak{U}_n\seq
\] 
where $\frak{U}_1,\ldots \frak{U}_n$ are arithmetic axiom formulae.''}
\end{center}
}
...
\bigskip
\begin{center}
{\em Add mathematical theories to first-order logic!}
\end{center}}
...
\emphdo{Add mathematical basic sequents}: 
...
\only<9>{
			\emphdb{Gentzen}: {\em Hauptsatz} doesn't extend to basic sequents as premises.
		}
...
\medskip
			\emphdr{Add non-logical rules of inference:}\semiproofadjust
			$$
...
\begin{overlayarea}{\textwidth}{5cm}			
		...
		\only<1->{
			Combining the classification of axioms into a \emphdb{polarities' hierarchy} (inspired by [Ciabattoni et al.])
			with a systematic construction of inference rules from axioms using \emphdb{focusing} [Andreoli],
			{justifies} the introduction of the class of \emphdr{bipolar axioms}.
		}
		
		\begin{itemize}
			\item<2-> \emphdb{Systematically} compute inference rules from
			{bipolar axioms} (\emphdr{$\lambda$-Prolog prototype});
			
			\item<3-> \emphdr{Uniform} presentation for \emphdb{classical} and \emphdb{intuitionistic} first order systems;
			
			\item<4-> \emphdb{Generalization} of the literature (e.g.~on \emphdr{geometric theories} [Negri et al, Vigan\`o, Simpson]);
			
			\item<5> \emphdr{Cut-elimination} \emphdb{guaranteed} for
