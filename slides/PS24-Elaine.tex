...
\only<10-11>{
		...
		\begin{itemize}
			\item<16-> \emphdb{Systematically} compute inference rules from
			{bipolar axioms} (\emphdr{$\lambda$-Prolog prototype});
			
			\item<17-> \emphdr{Uniform} presentation for \emphdb{classical} and \emphdb{intuitionistic} first order systems;
			
			\item<18-> \emphdb{Generalization} of the literature (e.g.~on \emphdr{geometric theories} [Negri et al, Vigan\`o, Simpson]);
			
			\item<19> \emphdr{Cut-elimination} \emphdb{guaranteed} for
