...
\author[Miller, Pimentel]{
  ...
  {\small Joint work with Elaine Pimentel, UCL} \\[50pt]
  {\small Based on a short talk given at The Proof Society Worskhop 2024}
}
\date{23 April 2026}
...
\emphdo{A naive attempt:} Expand the notion of initial rules to
include axioms.

\medskip

Assume $P\impl Q$ and $P$ as axioms. Allow initial rules to conclude
with these.
$$
  \infer[cut]
        {\vdash Q}
...
The {\em Hauptsatz} (i.e., cut-elimination) fails for systems with proper axioms.}
...
$$
  \infer[\emphdb{P\impl Q}]{\Gamma, P\seq C}{\Gamma, Q\seq C}
  \qquad
...
\textbf{{Polarization is a feature of linear logic: $\otimes$, $\&$,
    $\oplus$, $\parr$}}
\begin{itemize}
  \item A connective is \negf{negative} if its right-introduction rule is invertible.
  \item A connective is \negf{positive} if its left-introduction rule is invertible.
...
\begin{itemize}
\item In \emphdr{classical  logic}
\begin{itemize}
...
\begin{overlayarea}{\textwidth}{7cm}			
 ...
 \only<1>{\medskip
          \begin{center}
          \includegraphics[scale=0.8]{../slides/figs/h5}
          \end{center}}
...
\medskip
	...
	\node at (-1.5,-1.4) {$\infer[\kern -2pt D_l]{
			...
			\jLf{\Gamma,B}{B}{\Delta}}$};
	\node at (0,0) {\includegraphics[scale=0.65]{../slides/figs/focusing}};
		\end{tikzpicture}}
\only<3>{\medskip
\begin{tikzpicture}
	...
	\node at (-1.5,-1.4) {$\infer[\kern -2pt D_l]{
			\jUnf{\Gamma,B}{\cdot}{\cdot}{\Delta}}{
			\jLf{\Gamma,B}{B}{\Delta}}$};
        \node at (-2.2,0.7) {\includegraphics[scale=0.65]{../slides/figs/focusing2}};
...
\only<7-8>{
\node[draw] at (11,1) {$
	\infer[]{\Gamma, \emphdr{Q(t)}\seq\Delta}{
...
the \textsf{lub} predicate actually defines a function.  Encoding
functions as relations is another topic.
...
\pause Nonetheless, I illustrate some possible generalizations to
rules in the sequent calculus.
\end{frame}

\newcommand{\delayop}{\partial}
%\newcommand{\negdop}{\delayop}
...
\begin{frame}
\frametitle{Weak Law of Excluded Middle: $\neg \alpha\vee\neg\neg\alpha$}

Let $\negd{B}$ denote $B\wedgen\truen$ and $\neg B=B\impl\falsep$.
Both of these are always negative.

Polarized it as $\negd{\neg \alpha \veep \neg\neg \alpha}$, and assume
that the atomic formula $\alpha$ is negative.
...
\[
  ...
  \infer={\jLf{\Gamma_2}{\neg\neg \alpha}{C_2}}
         {\jUnf{\Gamma_2,\alpha}{}{}{}}
\]
This yields a higher-level rule of the form
\[
\infer{\Gamma\vdash C}
      {\deduce{\Gamma\vdash C\strut}
              {\deduce{\vdots}
                      {\left(~\vcenter{
                        \infer{\Gamma_1\vdash \alpha}{\strut}
                        }\right)}}
       \quad&\quad
       \deduce{\Gamma\vdash C\strut}
              {\deduce{\vdots}
                      {\left(~\vcenter{
                        \infer{\Gamma_2\vdash C_2}
                                   {\deduce{\Gamma_2,\alpha\vdash }{}}
                       }\right)}}}
\]
...
\begin{frame}
\frametitle{Perice's formula: $((\alpha\impl\beta)\impl\alpha)\impl\alpha$}

\[
  \infer{\jLf{\Gamma}{((\alpha\impl\beta)\impl\alpha)\impl\alpha}{\alpha}}
        {\infer{\jRf{\Gamma}{(\alpha\impl\beta)\impl\alpha}}
               {\infer={\jUnf{\Gamma}{}{(\alpha\impl\beta)\impl\alpha}{}}
                       {\jUnf{\Gamma,\alpha\impl\beta}{}{}{\alpha}}}
         &
         \infer{\jLf{\Gamma}{\alpha}{}{\alpha}}{}}
\]

Peirce's formula yields the restart rule.

\[
  \infer{\Gamma\vdash \alpha}
        {\infer{~\Gamma\vdash \alpha~}
               {\deduce{\vdots}{
                \left(~\vcenter{
...
\begin{frame}
\frametitle{Linearity axiom: $(\alpha\impl\beta)\vee(\beta\impl\alpha)$}


If $\alpha$ and $\beta$ are positive formulas, then this formula is an 
...
\infer{\Gamma\vdash C}
      {\deduce{\Gamma\vdash C\strut}
              {\deduce{\vdots}
                      {\infer{\Gamma_1,\alpha\vdash C_1}{\Gamma_1,\alpha,\beta\vdash C_1}}}\quad&\quad
       \deduce{\Gamma\vdash C\strut}
              {\deduce{\vdots}
                             {\infer{\Gamma_2,\beta\vdash C_2}{\Gamma_2,\beta,\alpha\vdash C_2}}}}
\]

One can see a resemblance with the following hypersequent inference rule.

\[
  \infer[com]
        {G\mid \alpha,\Gamma_1\vdash C_1\mid \beta,\Gamma_2\vdash C_2}
        {G\mid \alpha,\beta,\Gamma_1\vdash C_1 & G\mid  \alpha,\beta,\Gamma_2\vdash C_2}
\]
...
\begin{frame}
\frametitle{Issues with first-order quantification}

Consider the following $\mathcal{N}_4$ formulas
\[
  \forall x(\forall y(\forall u(P~x~y~u\impl S~x~y~u)\impl Q~x~y)\impl R~x
\]

\bigskip

\[
  \infer{\Gamma\vdash R x}
        {\deduce{\Gamma\vdash Q~x~y\strut}
                {\deduce{\vdots}
                        {\left(~\vcenter{
                              \infer{\Gamma'\vdash P~x~y~u}
                                    {\Gamma'\vdash S~x~y~u}}
                         \right)}}}
\]
...
One needs to invent new notation to show the scope and status of bound
variables: $y$ is a new (eigenvariable) while $u$ is a schema variable
local to the assume rule, etc.
...
\item Formally understand the connection between low complexity
  formulas (say, $\mathcal{N}_3$ and $\mathcal{N}_4$) proof systems
  such as two-level system of rules [Ciabattoni and Genco] and
  hypersequents.

\vfill
\item In mathematically oriented theories, what is the usual, informal
  practice?  (see, for example, the lub example).
