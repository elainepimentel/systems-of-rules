...
\infer[r]{\Gamma\vdash C}
      {\deduce{\Gamma\vdash C\strut}
              {\deduce{\vdots}
                      {\left(~\vcenter{
                        \infer[r_1]{\Gamma_1\vdash P}{\strut}
                        }\right)}}
       \quad&\quad
       \deduce{\Gamma\vdash C\strut}
              {\deduce{\vdots}
...
\DM{We should give a full definition of systems of rules.  2-systems
are a subset.  We should also mention the prior work
by \cite{schroeder-heister84} and \cite{avron90ndjfl}
and \cite{harper93jacm} of higher-level rules for natural deduction.
Maybe we should call our setting \emph{higher-level inference rules}.}
